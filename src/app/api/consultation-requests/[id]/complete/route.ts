...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const updated = await tx.consultationRequest.update({
      ...
      data: { status: RequestStatus.COMPLETED, version: { increment: 1 } },
    })
    await tx.requestStatusHistory.create({
      data: { consultationRequestId: params.id, previousStatus: request.status, newStatus: RequestStatus.COMPLETED, changedByUserId: user.id, source: 'ADMIN' },
    })
    return updated
  })

  await auditLog({ userId: user.id, action: 'COMPLETE_CONSULTATION_REQUEST', entityType: 'ConsultationRequest', entityId: params.id })
